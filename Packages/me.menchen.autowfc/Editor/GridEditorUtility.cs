...
namespace AutoWfc.Editor
{
    public static class GridEditorUtility 
    {
        ...
        private static MethodInfo _applyWireMaterialMethodInfo;
        
        // Use Reflection to access internal method
        public static void ApplyWireMaterial()
        {
            if (_applyWireMaterialMethodInfo is null)
            {
                var utilityType = typeof(HandleUtility);
                _applyWireMaterialMethodInfo = utilityType.GetMethod("ApplyWireMaterial",BindingFlags.Static);
            }
            _applyWireMaterialMethodInfo?.Invoke(null,null);
        }
        ...
        public static void DrawBox(Rect r, Color color)
        {
            ...
            Handles.matrix = GUI.matrix;
            Handles.color = color;
            Handles.DrawLine(new Vector3(r.xMin, r.yMin, 0f), new Vector3(r.xMax, r.yMin, 0f));
            Handles.DrawLine(new Vector3(r.xMax, r.yMin, 0f), new Vector3(r.xMax, r.yMax, 0f));
            Handles.DrawLine(new Vector3(r.xMax, r.yMax, 0f), new Vector3(r.xMin, r.yMax, 0f));
            Handles.DrawLine(new Vector3(r.xMin, r.yMax, 0f), new Vector3(r.xMin, r.yMin, 0f));
            Handles.matrix = oldMatrix;
        }
            ...
            Matrix4x4? localToWorldMatrix = null, float thickness = 0, Vector3 localOffset = default)
        {
            ...
            switch (gridLayout.cellLayout)
            {
                ...
                case GridLayout.CellLayout.Rectangle:
                    ...
                    var oldMatrix = Handles.matrix;
                    Handles.matrix = localToWorldMatrix ?? gridLayout.transform.localToWorldMatrix;
                    Handles.color = color;
                    // GUI.color = color;
                    int i = 0;
